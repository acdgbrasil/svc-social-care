...
struct PatientDetailedTests {
    ...
    func aggregatedIntelligence() throws {
        ...
        try patient.updateHealthStatus(health, actorId: "test-actor")
        ...
        try patient.updatePlacementHistory(history, actorId: "test-actor")
        ...
        try patient.updateIntakeInfo(ingress, actorId: "test-actor")
...
private func createValidPatient() throws -> Patient {
    ...
    let patient = try Patient(id: PatientId(), personId: pId, diagnoses: [try Diagnosis(id: try ICDCode("B201"), date: .now, description: "D", now: .now)], familyMembers: [prMember], prRelationshipId: prId, actorId: "test-actor")
    return patient
}
