...
const outPathInput =
  ...
  `${rootDir}/src/contracts/openapi.types.ts`;
...
async function main(): Promise<void> {
  ...
  for (const componentName of componentNames) {
    ...
    componentByTarget.set(internalPointerKey, componentName);
    ...
    if (isObject(schemaValue) && typeof schemaValue.$ref === "string") {
      ...
      componentByTarget.set(resolved.targetKey, componentName);
    }
  }
  ...
  for (const componentName of componentNames) {
    ...
    typeBlocks.push(
      `export type ${sanitizeTypeName(componentName)} = ${typeExpr};`,
    );
  }
  ...
  const schemaMapLines = componentNames.map(
    (componentName) =>
      `${JSON.stringify(componentName)}: ${sanitizeTypeName(componentName)};`,
  );
