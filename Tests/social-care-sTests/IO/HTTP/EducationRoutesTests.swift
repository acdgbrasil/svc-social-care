...
@Suite("Education Routes Specification")
struct EducationRoutesTests {

    @Test("GET /patients/{id}/education deve retornar perfis e vulnerabilidades")
    func testGetEducation() async throws {
        let patientId = UUID()
        let pId = PersonId()
        ...
        let mockPatient = Patient.reconstitute(
            ...
            educationalStatus: EducationalStatus(
                ...
                memberProfiles: [
                    MemberEducationalProfile(memberId: pId, canReadWrite: false, attendsSchool: false, educationLevelId: try LookupId(UUID().uuidString))
                ], 
                programOccurrences: []
            )
        )
        ...
        try await world.run { client in
            try await client.execute(uri: "/patients/\(patientId.uuidString)/education", method: .get) { response in
                #expect(response.status == .ok)
                let body = try JSONDecoder().decode(StandardResponse<EducationStatusResponseDTO>.self, from: response.body)
                #expect(body.status == "success")
                #expect(body.data.vulnerabilities.count > 0)
            }
        }
    }
}
