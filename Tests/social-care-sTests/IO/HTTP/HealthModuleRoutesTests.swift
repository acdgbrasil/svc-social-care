...
@Suite("Health Routes Specification")
struct HealthModuleRoutesTests {

    @Test("GET /patients/{id}/health deve retornar dados de saúde")
    func testGetHealth() async throws {
        ...
        try await world.run { client in
            try await client.execute(uri: "/patients/\(patientId.uuidString)/health", method: .get) { response in
                #expect(response.status == .ok)
                let body = try JSONDecoder().decode(StandardResponse<HealthStatusResponseDTO>.self, from: response.body)
                #expect(body.data.foodInsecurity == true)
            }
        }
    }
}
