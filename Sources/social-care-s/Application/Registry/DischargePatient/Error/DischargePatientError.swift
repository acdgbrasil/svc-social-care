...
extension DischargePatientError: AppErrorConvertible {
    ...
    public var asAppError: AppError {
        ...
        case .patientNotFound(let id):
            return appFailure("004", kind: "PatientNotFound", "Paciente não encontrado.", category: .domainRuleViolation, severity: .warning, http: 404, safeContext: ["patientId": id])
        case .alreadyDischarged(let id):
            return appFailure("001", kind: "AlreadyDischarged", "O paciente já está desligado.", category: .conflict, severity: .warning, http: 409, safeContext: ["patientId": id])
        ...
        case .cannotDischargeWaitlisted(let id):
            return appFailure("007", kind: "CannotDischargeWaitlisted", "Paciente em lista de espera não pode ser desligado. Use withdraw.", category: .conflict, severity: .warning, http: 409, safeContext: ["patientId": id])
        }
    }
