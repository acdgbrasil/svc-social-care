...
struct ICDCodeTests {
    ...
    struct CreationAndValidation {
        ...
        func createAllowsCodesWithoutDot() throws {
            let code = try ICDCode.create("A00", autoDot: false)
        ...
        func createFailsWithInvalidPattern() {
            #expect(throws: ICDCodeError.self) {
                try ICDCode.create("AA", requiresDot: true, autoDot: false)
            }
        }
    }
    ...
    struct Equality {
        ...
        func equality() throws {
            let code1 = try ICDCode.create("b201")
            let code2 = try ICDCode.create("B20.1")
