...
public enum UpdateWorkAndIncomeError: Error, Sendable, Equatable {
...
extension UpdateWorkAndIncomeError: AppErrorConvertible {
    ...
    private static let codePrefix = "UWI"
    ...
    private func appFailure(_ subCode: String, kind: String, _ message: String, category: AppError.Category, severity: AppError.Severity, http: Int, context: [String: Any] = [:]) -> AppError {
        AppError(
            ...
            safeContext: [:],
            observability: .init(category: category, severity: severity, fingerprint: ["\(Self.codePrefix)-\(subCode)"], tags: ["use_case": "update_work_and_income"]),
            http: http
        )
    }
}
