...
| 17 | Chave pública de IdP (JWKS) carregada **só no boot** é bug de disponibilidade: quando o IdP rotaciona a chave de assinatura, todo token novo traz um `kid` desconhecido e o serviço responde 401 em massa até o restart. Refresh **híbrido** obrigatório: (a) **periódico** — background task num `LifecycleHandler` (`didBoot` inicia, `shutdown` cancela) re-fetcha e aplica via **upsert por `kid`**; (b) **on-demand** — no `catch` do middleware, extrair o `kid` do header do token (base64url do 1º segmento, **sem** verificar assinatura — ainda não há chave para ele) e só disparar refresh se `kid ∉ knownKids` **e** passou o **cooldown**, retentando a verificação 1×. Anti-hammering é obrigatório: sem checar `kid` + cooldown, um burst de tokens inválidos vira fetch-storm contra o IdP. **NUNCA** substituir o set inteiro de chaves — durante a rotação o IdP serve a nova **e** a antiga, e trocar tudo quebra tokens em voo. Fetch atrás de um port (`JWKSFetching`) e **clock injetável** (ADR-034) para testar cooldown sem rede nem tempo real. O 401 permanece genérico (não vaza o motivo). | [ADR-040](../../../handbook/architecture/DECISIONS/ADR-040-jwks-runtime-refresh.md) | `Tests/.../IO/Auth/JWKSRefreshTest.swift` |
