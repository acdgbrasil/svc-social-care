...
struct DomainSpecificationTests {
    ...
    struct CommunitySupportNetworkSpec {
        ...
        func normalizeConflicts() throws {
            let csn = try CommunitySupportNetwork(hasRelativeSupport: true, hasNeighborSupport: true, familyConflicts: "  Test  ", patientParticipatesInGroups: true, familyParticipatesInGroups: true, patientHasAccessToLeisure: true, facesDiscrimination: false)
    ...
    struct DiagnosisSpec {
        ...
        private let icd = try! ICDCode("B20")
        ...
        func futureDate() throws {
            let future = try TimeStamp(Date().addingTimeInterval(1000))
            #expect(throws: DiagnosisError.self) {
                try Diagnosis(id: icd, date: future, description: "Test", now: now)
            }
        }
    ...
    struct SocialBenefitsCollectionSpec {
        ...
        func duplicates() throws {
            let fid = PersonId()
            let b1 = try SocialBenefit(benefitName: "A", amount: 100, beneficiaryId: fid)
            #expect(throws: SocialBenefitsCollectionError.duplicateBenefitNotAllowed(name: "A")) {
                try SocialBenefitsCollection([b1, b1])
            }
        }
        ...
        func totalSum() throws {
            let fid = PersonId()
            let b1 = try SocialBenefit(benefitName: "A", amount: 100, beneficiaryId: fid)
            let b2 = try SocialBenefit(benefitName: "B", amount: 200, beneficiaryId: fid)
