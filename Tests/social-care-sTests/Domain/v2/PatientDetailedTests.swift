...
struct PatientDetailedTests {
    ...
    func aggregatedIntelligence() throws {
        ...
        patient.updateHealthStatus(health)
        ...
        // 3. Intake
        let ingress = IngressInfo(ingressTypeId: try LookupId(UUID().uuidString), originName: "Origin", originContact: "Contact", serviceReason: "Reason", linkedSocialPrograms: [])
        patient.updateIntakeInfo(ingress)
...
private func createValidPatient() throws -> Patient {
    ...
    return try Patient(id: PatientId(), personId: pId, diagnoses: [try Diagnosis(id: try ICDCode("B201"), date: .now, description: "D", now: .now)], familyMembers: [prMember], prRelationshipId: prId)
}
